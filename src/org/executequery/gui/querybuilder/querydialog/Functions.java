...
public class Functions extends JDialog {
    ...
    private void initComboBox() {
        ...
        comboBoxFunction.addItemListener(event -> comboBoxFunction.setEditable(Objects.requireNonNull(comboBoxFunction.getSelectedItem()).toString().isEmpty()));
    }
    ...
    private void removeFunctions() {
        ...
        if (isSelectAll) {
            queryConstructor.replaceFunctions("", "stepBack");
            queryBuilderPanel.setTextInPanelOutputTestingQuery(queryConstructor.buildAndGetQuery());
            return;
        } else {
            for (JCheckBox checkBox : checkBoxesInScrollPane) {
                if (checkBox.isSelected()) {
                    if (stringBuilderFunctions.toString().contains(checkBox.getText())) {
                        stringBuilderFunctions.replace(stringBuilderFunctions.indexOf(checkBox.getText()),
                                stringBuilderFunctions.indexOf(checkBox.getText()) + checkBox.getText().length(),
                                "");

                        queryConstructor.replaceFunctions(stringBuilderFunctions.toString(), "stepBack");
                        queryBuilderPanel.setTextInPanelOutputTestingQuery(queryConstructor.buildAndGetQuery());
                    }
                }
            }
    ...
    private void addFunctions() {
        ...
        if (!Objects.requireNonNull(comboBoxFunction.getSelectedItem()).toString().isEmpty()) {
            if (stringBuilderAttributes.toString().equals("*") || stringBuilderAttributes.toString().isEmpty()) {
                if (queryConstructor.getFunctions().isEmpty()) {
                    ...
                    if (!queryConstructor.getFunctions().contains(stringBuilderFunctions)) {
                        queryConstructor.replaceFunctions(queryConstructor.getFunctions() + stringBuilderFunctions, "stepBack");
                        queryBuilderPanel.setTextInPanelOutputTestingQuery(queryConstructor.buildAndGetQuery());
                    }
                } else {
                    stringBuilderFunctions.append(comboBoxFunction.getSelectedItem().toString()).append("(").append(textFieldParamFunctions.getText()).append(")");
    ...
    private void arrangeCheckBoxesInScrollPane() {
        ...
        if (!queryConstructor.getFunctions().isEmpty()) {
            ...
            for (String noAlias : functionsNoAlias) {
                String[] functionsYesAlias = noAlias.split("(?<=\",)");
                for (String yesAlias : functionsYesAlias) {
                    JCheckBox checkBox = new JCheckBox(yesAlias);
        ...
        textField.getDocument().addDocumentListener(new DocumentListener() {
            ...
            private void textChanged() {
                ...
                for (String function : functions) {
                    if (function.contains(textFieldSearchFunctions.getText().toUpperCase())) {
