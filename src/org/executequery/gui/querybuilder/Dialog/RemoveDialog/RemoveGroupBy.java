...
/**
 * This class creates a dialog (window) to remove groupings from the query.
 * <p>
 * Этот класс создаёт диалог (окно) для удаления группировок из запроса.
 */
public class RemoveGroupBy extends JDialog{
    ...
    private JButton buttonRemoveGroupBy;
    private JTextField textFieldSearch;
    private JScrollPane scrollPanePlacingCheckBoxes;
    private ConnectionsComboBox connections;

    /**
     * A dialog (window) is created for deleting groupings.
     * <p>
     * Создаётся диалог (окно) для удаления группировок.
     */
    public RemoveGroupBy (QueryBuilderPanel queryBuilderPanel, QueryConstructor queryConstructor) {
    ...
    private void initButtons() {
        ...
        buttonRemoveGroupBy = WidgetFactory.createButton("buttonRemoveGroupBy", "Удалить", event -> {
            eventButtonRemoveGroupBy();
            closeDialog();
        });
    }
    ...
    private void addComponentsInPanelForPlacingComponents() {
        ...
        panelForPlacingComponents.add(buttonRemoveGroupBy, gridBagHelper.setXY(1, 3).setMinWeightX().setWidth(1).get());
    }
    ...
    private void addCheckBoxesInScrollPane() {
        ...
        ArrayList<String> historyGroupBy = queryConstructor.getHistoryGroupByAttributes();
    ...
    /**
     * A method for removing a grouping from a query.
     * <p>
     * Метод для удаления группировки из запроса.
     */
    private void eventButtonRemoveGroupBy() {
        ...
        for(int i = 0; i < checkBoxesInScrollPane.length; i++){
            if(checkBoxesInScrollPane[i].isSelected()){
                ...
                queryConstructor.getHistoryGroupByAttributes().remove(checkBoxesInScrollPane[i].getText());
                eventIfGroupByNotEmpty(stringBuilder);
                eventIfGroupByEmpty(stringBuilder);
            }
        }

        queryConstructor.replaceGroupBy(stringBuilder.toString());
        queryBuilderPanel.setTextInPanelOutputTestingQuery(queryConstructor.buildAndGetQuery());
        JOptionPane.showMessageDialog(queryBuilderPanel, "Аттрибуты группировки удалены", "Группировка", JOptionPane.ERROR_MESSAGE);
    }

    /**
     * Method (event) if the grouping is empty.
     * <p>
     * Метод (событие) если группировка пустая.
     */
    private void eventIfGroupByEmpty(StringBuilder stringBuilder) {
        if(queryConstructor.getHistoryGroupByAttributes().isEmpty()){
            if(stringBuilder.indexOf("GROUP BY") > 0){
    ...
    /**
     * Method (event) if the grouping is not empty.
     * <p>
     * Метод (событие) если группировка не пустая.
     */
    private void eventIfGroupByNotEmpty(StringBuilder stringBuilder) {
        if(!queryConstructor.getHistoryGroupByAttributes().isEmpty()) {
            String lastAttribute = queryConstructor.getHistoryGroupByAttributes().get(queryConstructor.getHistoryGroupByAttributes().size() - 1);
            ...
            if (lastAttribute.charAt(lastAttribute.length() - 1) == ',') {
                ...
                queryConstructor.getHistoryGroupByAttributes().set(queryConstructor.getHistoryGroupByAttributes().size() - 1, stringBuilderLastAttribute.toString());
