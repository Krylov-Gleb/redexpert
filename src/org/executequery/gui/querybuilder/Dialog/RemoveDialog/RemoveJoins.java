...
/**
 * A class for creating a dialog (window) for deleting connections.
 * <p>
 * Класс создания диалога (окна) для удаления соединений (Joins).
 *
 * @author Krylov Gleb
 */
public class RemoveJoins extends JDialog {
    ...
    private JButton buttonRemoveJoins;
    private JTextField textFieldSearch;
    private JScrollPane scrollPanePlacingCheckBoxes;
    private ConnectionsComboBox connections;

    /**
     * A dialog (window) is created to delete connections (joins).
     * <p>
     * Создаётся диалог (окно) для удаления соединений (joins).
     */
    public RemoveJoins(QueryBuilderPanel queryBuilderPanel, QueryConstructor queryConstructor) {
    ...
    private void initButtons() {
        ...
        buttonRemoveJoins = WidgetFactory.createButton("buttonRemoveJoins", "Удалить", event -> {
            eventButtonRemoveJoins();
            closeDialog();
        });
    }
    ...
    private void addComponentsInPanelForPlacingComponents() {
        ...
        panelForPlacingComponents.add(buttonRemoveJoins, gridBagHelper.setXY(1, 3).setMinWeightX().setWidth(1).get());
    }
    ...
    private void arrangeCheckBoxesInScrollPane() {
        ...
        ArrayList<String> historyAddJoins = queryConstructor.getHistoryJoin();

        for (int i = 0; i < historyAddJoins.size(); i++) {
            panelArrangeCheckBoxInScrollPane.add(new JCheckBox(historyAddJoins.get(i)));
        }
    ...
    /**
     * A method for removing joins from a request.
     * <p>
     * Метод для удаления соединений (joins) из запроса.
     */
    private void eventButtonRemoveJoins() {
        ...
        for (int i = 0; i < checkBoxesInScrollPane.length; i++) {
            if (checkBoxesInScrollPane[i].isSelected()) {
                queryConstructor.removeJoin(checkBoxesInScrollPane[i].getText(), queryConstructor.getHistoryJoinNameTable().get(i));
            }
        }

        queryConstructor.addAttributes(queryBuilderPanel.getListTable());
        queryBuilderPanel.setTextInPanelOutputTestingQuery(queryConstructor.buildAndGetQuery());
        JOptionPane.showMessageDialog(queryBuilderPanel, "Соединения удаленны!", "Соединение (Join)", JOptionPane.ERROR_MESSAGE);
    }
